...
fn partialdec(ct1: &Vec<G1>, ct2: &Vec<G1>, sk: F) {
    let agg_ct1 = ct1.iter().fold(G1::zero(), |acc, c| acc + c);
    let agg_ct2 = ct2.iter().fold(G1::zero(), |acc, c| acc + c);

    let _partial_decryption = agg_ct2 - (agg_ct1 * sk);
}

fn bench_beatmevpd(c: &mut Criterion) {
    let mut rng = test_rng();
    let sk = F::rand(&mut rng);

    let mut group = c.benchmark_group("beatmev_pd");
    for &batch_size in [8usize, 32, 128, 512].iter() {
        let ct1 = (0..batch_size)
            .map(|_| G1::rand(&mut rng))
            .collect::<Vec<_>>();

        let ct2 = (0..batch_size)
            .map(|_| G1::rand(&mut rng))
            .collect::<Vec<_>>();
...
criterion_group!(benches, bench_beatmevpd);
